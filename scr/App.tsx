...
// Wapilocs — Web Prototype (Preview) v2.0
// New in v2.0:
// - Escrow simulation (Paid — blocked) with OTP/QR-like code
// - Flow: Pending → Confirmed → Paid (blocked) → Delivered (OTP) → Completed
// - "Report a problem" + Auto-refund timer (Admin-configurable)
// - Commission visibility control (none / owners / renters / all)
// - Better validation with field hints
...
const I18N = {
  en: {
    ...
    post: "Post",
    listings: "Listings",
    missing: "Please fill title, description, phone and payment mode.",
    contact: "Contact",
    ...
    refundIn: function(h){return "Auto-refund in "+h+"h if no delivery";},
  ...
  fr: {
    ...
    post: "Publier",
    listings: "Annonces",
    missing: "Veuillez remplir le titre, la description, le téléphone et le mode de paiement.",
    contact: "Contacter",
    ...
    refundIn: function(h){return "Remboursement auto dans "+h+"h si pas de livraison";},
...
function breakdown(amount, rate){
...
const Chip = ({ label, selected, onClick, color = "border-slate-300" }) => (
  <button onClick={onClick} className={`px-3 py-1 rounded-full border ${selected ? "border-emerald-600 bg-emerald-50 font-semibold" : color} mr-2 mb-2 text-sm`}>{label}</button>
);

const Badge = ({ children }) => (
  <span className="inline-block text-xs px-2 py-1 rounded-full bg-slate-100 border border-slate-200 mr-2 mb-2">{children}</span>
);

const Logo = ({ url }) => {
  if (url) return <img src={url} alt="Wapilocs logo" className="h-8 w-auto rounded"/>;
...
const StatusPill = ({ status, t }) => {
  ...
  const s = map[status];
  return <span className={`ml-2 px-2 py-0.5 text-xs rounded-full border ${s.cls}`}>{s.label}</span>;
};

const ListingCard = ({ item, t, onOpenChat, commissionRate, onAsk, onConfirm, onPay, onShowOtp, onValidateOtp, onReport, showCommission, autoRefundHours }) => {
  ...
  return (
    <div className="border-b border-slate-200 py-4">
      ...
      <div className="text-xs opacity-70 mt-1">{t.catLabel[item.category] || item.category} | {item.payment || "-"}</div>
      {item.imageUrl && (
        <img src={item.imageUrl} alt="item" className="w-full h-48 object-cover rounded-xl mt-2"/>
      )}
      <div className="mt-2">{item.description}</div>
      {item.status === 'paid_blocked' && (
      ...
      {item.category === 'moving' && item.moving && (
...
export default function App(){
  ...
  useEffect(()=>{
    try{ const saved = localStorage.getItem('chs_admin_config'); if(saved){ setConfig({ ...DEFAULT_CONFIG, ...JSON.parse(saved) }); } }catch(e){ console.warn('Config load failed', e); }
  },[]);
  useEffect(()=>{ try{ localStorage.setItem('chs_admin_config', JSON.stringify(config)); }catch(e){} },[config]);

  const [lang, setLang] = useState("en");

  // Auto-detect language on first load; remember choice
  useEffect(() => {
  ...
  useEffect(() => { localStorage.setItem("chs_lang", lang); }, [lang]);

  const t = useMemo(()=> I18N[lang], [lang]);
  const commissionRate = config.commission;
  const showCommissionForOwners = config.commissionVisibility === 'owners' || config.commissionVisibility === 'all';
  const showCommissionForRenters = config.commissionVisibility === 'renters' || config.commissionVisibility === 'all';
  const enabledPayments = useMemo(()=> PAYMENTS.filter(p=> config.paymentsEnabled[p]), [config]);
  const visibleCats = useMemo(()=> CATEGORY_SLUGS.filter(slug=> config.visibleCategories[slug]), [config]);
  ...
  const [moving, setMoving] = useState({ forfait:"", extraHour:"", extraKm:"", floors:0, elevator:false, team:2, area:"", options:{packing:false, assembly:false, cleaning:false}, deposit:"" });
  const [btp, setBtp] = useState({ machineType:"", tonnage:"", bucket:"", withOperator:true, mobilisation:"", minDays:1, zone:"", dailyRate:"", deposit:"" });
  ...
  useEffect(()=>{ if(!config.paymentsEnabled[payment]){ setPayment(enabledPayments[0] || PAYMENTS[0]); } }, [config, enabledPayments, payment]);
  useEffect(()=>{ if(!config.visibleCategories[category]){ setCategory(visibleCats[0] || CATEGORY_SLUGS[0]); } }, [config, visibleCats, category]);

  // --- Persistence: listings + chatMap ---
  const LS_LISTINGS_KEY = 'chs_listings_v20';
  const LS_CHATMAP_KEY = 'chs_chatmap_v20';

  const DEFAULT_LIST = [
    { id: "1", title: "Generator 8kVA for rent", description: "Reliable diesel generator, daily hire.", price: "10,000 FC/day", phone: "+243970000001", imageUrl: "https://images.unsplash.com/photo-1581091012167-5b2b86b59744?q=80&w=1200&auto=format&fit=crop", category: "generators", payment: "Airtel Money", status: 'available' },
    { id: "2", title: "Motorbike for short hire", description: "Perfect for errands around town.", price: "25,000 FC/day", phone: "+243970000002", imageUrl: "https://images.unsplash.com/photo-1518655048521-f130df041f66?q=80&w=1200&auto=format&fit=crop", category: "transport", payment: "M-Pesa", status: 'available' },
    { id: "3", title: "Loader CAT 950GC (with operator)", description: "18t loader, 3.1 m³ bucket. Operator included. Mobilisation extra.", price: "450,000 FC/day", phone: "+243970000003", imageUrl: "https://images.unsplash.com/photo-1581092795360-fd1f78d244a1?q=80&w=1200&auto=format&fit=crop", category: "heavy_btp", payment: "Airtel Money", status: 'available', btp: { machineType:"Loader 950GC", tonnage:"18t", bucket:"3.1 m³", withOperator:true, mobilisation:"100,000 FC", minDays:1, zone:"Lubumbashi", dailyRate:"450,000 FC", deposit:"" } },
    { id: "4", title: "Déménagement — 2 hommes + pickup", description: "Chargement, transport, déchargement. Lubumbashi et communes voisines.", price: "120,000 FC/jour", phone: "+243970000004", imageUrl: "https://images.unsplash.com/photo-1598300183870-1db9f2021f5c?q=80&w=1200&auto=format&fit=crop", category: "moving", payment: "Orange Money", status: 'available', moving: { forfait:"120,000 FC / 3h / 10km", extraHour:"20,000 FC", extraKm:"2,000 FC", floors:1, elevator:false, team:2, area:"Lushi centre", options:{packing:true, assembly:false, cleaning:false}, deposit:"" } }
  ];

  const [listings, setListings] = useState(DEFAULT_LIST);
  const [chatMap, setChatMap] = useState({}); // { [listingId]: [{id, sender, text}] }
  ...
  useEffect(()=>{ try{ localStorage.setItem(LS_LISTINGS_KEY, JSON.stringify(listings)); }catch(e){} }, [listings]);
  useEffect(()=>{ try{ localStorage.setItem(LS_CHATMAP_KEY, JSON.stringify(chatMap)); }catch(e){} }, [chatMap]);
  ...
  const filtered = useMemo(()=>{
    ...
    return listings.filter(it => {
      const byCat = activeCat === "All" || activeCat === t.all || it.category === activeCat;
      const byTerm = !term || (String(it.title||"").toLowerCase().includes(term) || String(it.description||"").toLowerCase().includes(term));
      return byCat && byTerm;
    });
  },[listings, activeCat, search, t]);

  function validate(){
    const errs = {};
    if(!title.trim()) errs.title = t.required.title;
    if(!description.trim()) errs.desc = t.required.desc;
    if(!phone.trim()) errs.phone = t.required.phone; else if(!isValidPhone(phone)) errs.phone = t.invalid.phone;
    if(!payment) errs.payment = t.required.payment;
    setErrors(errs);
    if(Object.keys(errs).length){
      if(errs.title && titleRef.current){ titleRef.current.scrollIntoView({behavior:'smooth', block:'center'}); }
      else if(errs.phone && phoneRef.current){ phoneRef.current.scrollIntoView({behavior:'smooth', block:'center'}); }
      return false;
    }
    return true;
  }
  ...
  function postListing(){
    ...
    const payload = { id: String(Date.now()), title: title.trim(), description: description.trim(), price: price.trim() || null, phone: phone.trim(), imageUrl: chosenImage, category, payment, status: 'available' };
    if(category === 'moving') payload.moving = { ...moving };
    if(category === 'heavy_btp') payload.btp = { ...btp };
    setListings([payload, ...listings]);
    setTitle(""); setDescription(""); setPrice(""); setPhone(""); setImageUrl(""); setImageFilePreview(""); setErrors({});
    setCategory(visibleCats[0] || CATEGORY_SLUGS[0]); setPayment(enabledPayments[0] || PAYMENTS[0]);
    setMoving({ forfait:"", extraHour:"", extraKm:"", floors:0, elevator:false, team:2, area:"", options:{packing:false, assembly:false, cleaning:false}, deposit:"" });
    setBtp({ machineType:"", tonnage:"", bucket:"", withOperator:true, mobilisation:"", minDays:1, zone:"", dailyRate:"", deposit:"" });
  }

  const liveAmt = parseCDF(price);

  // --- Booking actions (mock) ---
  function askBooking(item){
  ...
  function payBooking(item){
    const otp = genOTP();
    const paidAt = Date.now();
    setListings(listings.map(x => x.id === item.id ? { ...x, status: 'paid_blocked', otp, paidAt } : x));
  }
  ...
  function openChat(it){
    ...
    setChatOpen(true);
    if(!chatMap[it.id]){
      setChatMap({ ...chatMap, [it.id]: [{ id: 'm1', sender: 'owner', text: lang==='fr'? 'Bonjour ! Disponible.' : 'Hi! Item is available.' }] });
    }
  }
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="px-4 sm:px-6 lg:px-8 pt-6 pb-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold">{t.appTitle} <span className="ml-2 text-xs font-normal px-2 py-1 border rounded-full">v2.0</span></h1>
            <div className="text-sm text-slate-600">{t.slogan}</div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={()=> setLang(lang === "en" ? "fr" : "en")} className="px-3 py-1 rounded-lg border border-slate-300 font-semibold">{t.lang}</button>
          <button onClick={()=> setAdminOpen(true)} className="px-3 py-1 rounded-lg border border-amber-500 text-amber-700">Admin</button>
        </div>
      </div>
      ...
      <div className="px-4 sm:px-6 lg:px-8 mb-2">
        { ["feed","map"].map(key => (
          <Chip key={key} label={ key === "feed" ? t.tabFeed : t.tabMap } selected={tab===key} onClick={()=> setTab(key)} />
        )) }
      </div>
        ...
        <div className="px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1">
            <div className="p-4 border rounded-2xl shadow-sm">
              <input ref={titleRef} value={title} onChange={e=> setTitle(e.target.value)} placeholder={t.titlePh} className={`w-full border rounded-lg px-3 py-2 mb-1 ${errors.title ? 'border-red-500' : 'border-slate-300'}`}/>
              ...
              <div className="grid grid-cols-2 gap-2">
                <input value={price} onChange={e=> setPrice(e.target.value)} placeholder={t.pricePh} className="border border-slate-300 rounded-lg px-3 py-2"/>
                <input ref={phoneRef} value={phone} onChange={e=> setPhone(e.target.value)} placeholder={t.phonePh} className={`border rounded-lg px-3 py-2 ${errors.phone ? 'border-red-500' : 'border-slate-300'}`} inputMode="tel"/>
              </div>
              ...
              {showCommissionForOwners && (
                <div className="text-xs text-slate-600 mt-2">
                  {liveAmt ? (
                    <>
                      {t.commissionLabel}: <span className="font-semibold">{Math.round(liveAmt*commissionRate).toLocaleString()} FC</span> • {t.payoutLabel}: <span className="font-semibold">{(liveAmt - Math.round(liveAmt*commissionRate)).toLocaleString()} FC</span>
                    </>
                  ) : (
                    <span>{t.feeSimHint}</span>
                  )}
                </div>
              )}
              ...
              <input type="file" accept="image/*" capture="environment" onChange={(e)=>{ const f = e.target.files?.[0]; if(!f) return; const r = new FileReader(); r.onload = ev=> setImageFilePreview(String(ev.target?.result||"")); r.readAsDataURL(f); }} className="w-full mb-2"/>
              ...
              <div>
                ...
                <div className="flex flex-wrap">
                  <Chip label={t.all} selected={activeCat === "All" || activeCat === t.all} onClick={()=> setActiveCat("All")} />
              ...
              <button onClick={postListing} className="w-full mt-3 bg-emerald-600 text-white py-2 rounded-lg">{t.post}</button>
            </div>
          </div>
          ...
          <div className="lg:col-span-2">
            ...
            <div className="divide-y">
              {filtered.map(item => (
                <ListingCard key={item.id} item={item} t={t} onOpenChat={openChat} commissionRate={commissionRate} onAsk={askBooking} onConfirm={confirmBooking} onPay={payBooking} onShowOtp={showOtp} onValidateOtp={validateOtp} onReport={reportProblem} showCommission={showCommissionForRenters} autoRefundHours={config.autoRefundHours} />
              ))}
              {filtered.length === 0 && (
                <div className="text-slate-500 py-8">{lang==='fr' ? 'Aucune annonce. Soyez le premier à publier.' : 'No items yet. Be the first to post.'}</div>
              )}
            </div>
          </div>
        </div>
      ...
      {tab === "map" && (
        <div className="px-4 sm:px-6 lg:px-8">
          ...
          {/* Map placeholder */}
          <div className="w-full h-96 rounded-2xl border border-slate-200 grid place-items-center text-slate-500">
              ...
              <div className="border rounded-xl p-3">
                ...
                {[
                  ...
                  <label key={opt.key} className="flex items-center gap-2 text-sm mb-1">
                    <input type="radio" name="commvis" checked={config.commissionVisibility===opt.key} onChange={()=> setConfig({...config, commissionVisibility: opt.key})} />
                    {opt.label}
                  </label>
                ))}
              </div>
              ...
              <div className="border rounded-xl p-3">
                ...
                {PAYMENTS.map(p => (
                  <label key={p} className="flex items-center gap-2 text-sm mb-1">
                    <input type="checkbox" checked={!!config.paymentsEnabled[p]} onChange={e=> setConfig({...config, paymentsEnabled: {...config.paymentsEnabled, [p]: e.target.checked }})}/>
                    {p}
                  </label>
                ))}
              </div>
              ...
              <div className="border rounded-xl p-3">
                ...
                {CATEGORY_SLUGS.map(slug => (
                  <label key={slug} className="flex items-center gap-2 text-sm mb-1">
                    <input type="checkbox" checked={!!config.visibleCategories[slug]} onChange={e=> setConfig({...config, visibleCategories: {...config.visibleCategories, [slug]: e.target.checked }})}/>
                    {t.catLabel[slug]}
                  </label>
                ))}
              </div>
...
function DynFields({ t, category, moving, setMoving, btp, setBtp }){
  const d = t.dyn;
  if(category === 'moving'){
    return (
      <div className="mt-3 border rounded-xl p-3">
        ...
        <div className="mt-2">
          <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={moving.elevator} onChange={e=> setMoving({...moving, elevator:e.target.checked})}/> {d.elevator}</label>
        </div>
  ...
  if(category === 'heavy_btp'){
    return (
      <div className="mt-3 border rounded-xl p-3">
        ...
        <div className="mt-2">
          <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={btp.withOperator} onChange={e=> setBtp({...btp, withOperator:e.target.checked})}/> {d.withOperator}</label>
        </div>
        <div className="grid grid-cols-2 gap-2 mt-2">
...
console.assert(['none','owners','renters','all'].includes(DEFAULT_CONFIG.commissionVisibility), 'commissionVisibility must be one of none/owners/renters/all');
